...
for year in range(2004, 2024):
    ...
    row.loc[0, 'oppW%_mid50'] = (data.loc[data['Season'] == year, 'oppW%'].sort_values().iloc[22] + data.loc[data['Season'] == year, 'oppW%'].sort_values().iloc[21])/2 - (data.loc[data['Season'] == year, 'oppW%'].sort_values().iloc[8] + data.loc[data['Season'] == year, 'oppW%'].sort_values().iloc[7])/2
    row.loc[0, 'W%_stdev'] = data.loc[data['Season'] == year, 'W%'].std() # standard deviation
    row.loc[0, 'W%_range'] = data.loc[data['Season'] == year, 'W%'].max() - data.loc[data['Season'] == year, 'W%'].min() # range
    row.loc[0, 'W%_mid50'] = (data.loc[data['Season'] == year, 'W%'].sort_values().iloc[22] + data.loc[data['Season'] == year, 'W%'].sort_values().iloc[21])/2 - (data.loc[data['Season'] == year, 'W%'].sort_values().iloc[8] + data.loc[data['Season'] == year, 'W%'].sort_values().iloc[7])/2
...
sns.lineplot(x=np.arange(2004,2024,1).astype(int), y=df['W%_stdev'], color='red', alpha=0.8)
plt.ylim((0, 0.024))
...
sns.lineplot(x=np.arange(2004,2024,1).astype(int), y=df['W%_range'], color='red', alpha=0.8)
...
sns.lineplot(x=np.arange(2004,2024,1).astype(int), y=df['W%_mid50'], color='red', alpha=0.8)
plt.ylim((0, 0.024))
plt.xticks(np.arange(2004,2024,1).astype(int), rotation=60)
